...
class Scenario(BaseScenario):
	...
	def random_scenario(self, world):
		...
		# Routes for the double-Y:
		#   Start on tube 0 → [0, 2, 4]
		#   Start on tube 1 → [1, 2, 4]
		#   Start on tube 3 → [3, 4]
		starting_tubes = [0, 1, 3]  # the three entry branches
		route_map = {
			0: [0, 2, 4],
			1: [1, 2, 4],
			3: [3, 4],
		...
		while True:
			...
			# Round-robin across the three entry branches (tubes 0, 1, 3)
			start_tube = starting_tubes[num_agents_added % len(starting_tubes)]
			current_tube = world.tube_params[start_tube]
			self.current_tube[num_agents_added] = start_tube
			self.tube_route[num_agents_added] = route_map[start_tube]
			...
			along_offset = -(num_agents_added // len(starting_tubes) + 1) * long_spacing
	...
	# setup double-Y configuration: two sequential Y-merges
	def setup_two_tube_params(self, world):
		"""
		Double-Y: 5 corridors with two sequential Y-shaped merges.

		Layout (schematic):
		  Tube 0 (UL)   Tube 1 (UR)        ← First Y branches
		       \\         /
		        merge1
		          \\
		       Tube 2  (stem of Y1 = left branch of Y2)
		            \\        Tube 3 (right branch of Y2)
		             \\      /
		             merge2
		               |
		             Tube 4  (final stem, straight down)

		Routes:
		  Agents starting on tube 0: [0, 2, 4]
		  Agents starting on tube 1: [1, 2, 4]
		...
		final_stem_len = ws * 0.30   # tube 4 (final exit stem)

		# ---- Second Y (lower) — stem goes straight down ----
		y2_half_angle = np.pi / 6   # 30° branch spread
		left_out2  = np.array([-np.sin(y2_half_angle),  np.cos(y2_half_angle)])
		right_out2 = np.array([ np.sin(y2_half_angle),  np.cos(y2_half_angle)])

		# Place merge2 so the overall layout is roughly centred vertically
		merge2 = np.array([0.0, -ws * 0.05])

		# merge1 sits at the far end of Y2's left branch direction
		merge1 = merge2 + left_out2 * (connect_length + 2 * gap)

		# ---- First Y (upper) — branches fan out from the stem axis ----
		stem1_vec = merge2 - merge1
		stem1_dir = stem1_vec / np.linalg.norm(stem1_vec)
		outward1 = -stem1_dir                        # away from stem toward branches
		outward1_angle = float(np.arctan2(outward1[1], outward1[0]))

		y1_half_angle = np.pi / 5   # 45° branch spread (wider than Y2)
		left_out1  = np.array([np.cos(outward1_angle + y1_half_angle),
							   np.sin(outward1_angle + y1_half_angle)])
		right_out1 = np.array([np.cos(outward1_angle - y1_half_angle),
							   np.sin(outward1_angle - y1_half_angle)])
		...
		# === TUBE 0: Y1 left branch → merge1 ===
		ent0 = merge1 + left_out1 * (branch_length + gap)
		ext0 = merge1 + left_out1 * gap
		world.tube_params.append(_make_tube(ent0, ext0))

		# === TUBE 1: Y1 right branch → merge1 ===
		ent1 = merge1 + right_out1 * (branch_length + gap)
		ext1 = merge1 + right_out1 * gap
		world.tube_params.append(_make_tube(ent1, ext1))

		# === TUBE 2: merge1 → merge2 (stem of Y1 / left branch of Y2) ===
		ent2 = merge1 + stem1_dir * gap
		ext2 = merge2 + left_out2 * gap
		world.tube_params.append(_make_tube(ent2, ext2))

		# === TUBE 3: Y2 right branch → merge2 ===
		ent3 = merge2 + right_out2 * (branch_length + gap)
		ext3 = merge2 + right_out2 * gap
		world.tube_params.append(_make_tube(ent3, ext3))

		# === TUBE 4: merge2 → exit (final stem, straight down) ===
		ent4 = merge2 + np.array([0.0, -gap])
		ext4 = merge2 + np.array([0.0, -(final_stem_len + gap)])
