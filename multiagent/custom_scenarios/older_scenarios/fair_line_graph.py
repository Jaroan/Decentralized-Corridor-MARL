...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		self.total_sep = 1.0*self.world_size
	...
	def reset_world(self, world:World) -> None:
		...
		for i, agent in enumerate(world.agents):
			...
			if i%3 == 0:
				agent.color = np.array([0.35, 0.35, 0.95])
			elif i%3 == 1:
	...
	def random_scenario(self, world):
		...
		world.landmarks[0].state.p_pos = np.random.uniform(-.15*self.world_size, .15*self.world_size, world.dim_p)
		# world.landmarks[0].state.p_pos = np.array([-0.9, 0.9])
		world.landmarks[0].state.p_vel = np.zeros(world.dim_p)
		# print("landmark 0",world.landmarks[0].state.p_pos)
		...
		loc = landmark_pose + self.total_sep*np.array([np.cos(theta), np.sin(theta)])
		# print("loc",loc, "theta",theta)
		# find a suitable theta such that landmark 1 is within the bounds
		# print("check",not(abs(loc[0])<self.world_size and abs(loc[1])<self.world_size))
		while not(abs(loc[0])<self.world_size and abs(loc[1])<self.world_size):
			theta += np.radians(5)
			# print("theta",theta)
		...
		# print("landmark 1",world.landmarks[1].state.p_pos)
		self.expected_poses = np.array([world.landmarks[0].state.p_pos + i*self.ideal_sep*np.array([np.cos(theta), np.sin(theta)]) 
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		# Calculate the vector between the two landmarks
		landmark_vector = world.landmarks[1].state.p_pos - world.landmarks[0].state.p_pos
		# Calculate the vector between the agent and the first landmark
		agent_vector = agent.state.p_pos - world.landmarks[0].state.p_pos
		# Calculate the dot product of the two vectors
		dot_product = np.dot(landmark_vector, agent_vector)
		# Calculate the magnitude (length) of the two vectors
		magnitude_landmark_vector = np.linalg.norm(landmark_vector)
		magnitude_agent_vector = np.linalg.norm(agent_vector)

		# Calculate the angle between the two vectors
		cos_angle = dot_product / (magnitude_landmark_vector * magnitude_agent_vector)
		angle = np.arccos(np.clip(cos_angle, -1, 1))

		# Calculate the perpendicular distance from the agent to the line
		perpendicular_distance = magnitude_agent_vector * np.sin(angle)

		# Define a threshold for the maximum allowed perpendicular distance
		threshold = 0.1
		# Check if the agent is on the line between the two landmarks
		if perpendicular_distance <= threshold:
		...
		# print("agent",agent.id,"dist_left_to_goal",world.dist_left_to_goal[agent.id])
	...
	# 	# print("DONE",self.is_success or condition1)
	...
	def reward(self, agent, world):
		...
		if agent.id == 0:
			...
			# # Update only the relevant indices in expected_poses_occupied
			# self.expected_poses_occupied[mask] = 1.0
			...
			if self.goal_match_index.size == 0 or self.goal_match_index.shape != (self.num_agents,):
				self.goal_match_index = np.arange(self.num_agents)
		dist_to_fair_goal = np.linalg.norm(agent.state.p_pos - self.expected_poses[self.goal_match_index[agent.id]])
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		sorted_indices = np.argsort(world.dists)
		...
		if min_dist < self.min_obs_dist:
			...
			if min_dist < self.min_dist_thresh:
				...
				# print("Ag",agent.id," AT GOAL",np.min(world.dists), "goal_occupied",self.expected_poses_occupied[chosen_goal])
			...
			if len(unoccupied_goals) > 0:
				...
				## determine which goal from self.expected_poses is this chosen unocccupied goal
				## use the index of the unoccupied goal to get the goal from self.expected_poses
