...
class Scenario(BaseScenario):
	...
	def reward(self, agent, world):
		...
		if agent.id == 0:
			...
			# # Define the angle of the arrow's wings
			# # arrow_angle = np.random.uniform(0, np.pi)  # Adjust the range as needed
			...
			# print("reward expected_poses_occupied",self.expected_poses_occupied)
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		if np.min(world.dists) < self.min_dist_thresh:
			...
			agents_goal = self.expected_poses[np.argmin(world.dists)]
			# update self.expected_poses_occupied to indicate that the goal is occupied
			self.expected_poses_occupied[np.argmin(world.dists)] = 1
			# print("********** goal reached **********")
			# print("obs expected_poses",self.expected_poses_occupied,np.argmin(world.dists) )
			goal_occupied = np.array([self.expected_poses_occupied[np.argmin(world.dists)]])
			# print("obs goal_occupied flag",goal_occupied)
		else:
			# print("obs not reached expected_poses",self.expected_poses_occupied)
			unoccupied_goals = self.expected_poses[self.expected_poses_occupied==0]
			# print("obs unoccupied_goals",unoccupied_goals,self.expected_poses_occupied)
			if len(unoccupied_goals) > 0:
				...
				# print("obs goal_occupied flag",goal_occupied)
				# world.dists = np.array([np.linalg.norm(agent.state.p_pos - l) for l in unoccupied_goals])
				# agents_goal = unoccupied_goals[np.argmin(world.dists)]
				# goal_occupied = np.array([0])
			else:
				...
				# print("!!!!!!!goal_occupied flag",self.expected_poses_occupied[goals[agent.id]])
	...
	def _get_entity_feat_relative(self, agent:Agent, entity:Entity, world:World) -> arr:
		...
		if 'agent' in entity.name:
			...
			if np.min(world.dists) < self.min_dist_thresh:
				# If the minimum distance is already less than self.min_dist_thresh, use the previous goal.
				# print("~~~~~~~~Rel goal reached~~~~~~~~~~~~",self.expected_poses_occupied)
				self.expected_poses_occupied[np.argmin(world.dists)] = 1
