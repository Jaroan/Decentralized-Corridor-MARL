...
class Scenario(BaseScenario):
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		# print(world.landmarks[0].state.p_pos,"lobe1_center",lobe1_center,"lobe2_center",lobe2_center)
		...
		# Calculate the distance to the central landmark
		# distance_to_center = np.linalg.norm(agent.state.p_pos - world.landmarks[0].state.p_pos)
		...
		if distance_to_center1 < 2.5*(self.target_radius/2) or distance_to_center2 < 2.5*(self.target_radius/2):
	...
	def reward(self, agent, world):
		...
		tanh_output = np.tanh(scaled_input-5.0)
		fair_rew = self.fair_rew * tanh_output
		rew += fair_rew 
		# print("fair_rew",fair_rew,"rew",rew)
		rew = np.clip(rew, -2*self.collision_rew, self.goal_rew+self.fair_rew)
		return rew
	...
	def observation(self, agent:Agent, world:World) -> arr:
		...
		if np.min(world.dists) < self.min_dist_thresh:
			...
			if len(unoccupied_goals) > 0:
				...
				##### try fair assignment to check if it makes it more fair
				agent_pos = [agent.state.p_pos for agent in world.agents]

				costs = dist.cdist(agent_pos, self.expected_poses)
				x, objs = solve_fair_assignment(costs)
				# print('observations x',x, objs)
				self.goal_match_index = np.where(x==1)[1]
				# print("goalmatch",self.goal_match_index)
				if self.goal_match_index.size == 0 or self.goal_match_index.shape != (self.num_agents,):
					self.goal_match_index = np.arange(self.num_agents)
				agents_goal = self.expected_poses[self.goal_match_index[agent.id]]
				##check if the goal is occupied
				goal_occupied = np.array([self.expected_poses_occupied[self.goal_match_index[agent.id]]])
				# # self.delta_dists = self._bipartite_min_dists(self.dists)
				# # print("delta_dists",self.delta_dists)
		...
		if 'agent' in entity.name:
			...
			if np.min(world.dists) < self.min_dist_thresh:
				...
				if len(unoccupied_goals) > 0:
					...
					##### try fair assignment to check if it makes it more fair
					agent_pos2 = [agent.state.p_pos for agent in world.agents]

					costs = dist.cdist(agent_pos2, self.expected_poses)
					x, objs = solve_fair_assignment(costs)
					# print('observations x',x, objs)
					self.goal_match_index = np.where(x==1)[1]
					# print("goalmatch",self.goal_match_index)
					if self.goal_match_index.size == 0 or self.goal_match_index.shape != (self.num_agents,):
						self.goal_match_index = np.arange(self.num_agents)
					goal_pos = self.expected_poses[self.goal_match_index[agent.id]]
					##check if the goal is occupied
					goal_occupied = np.array([self.expected_poses_occupied[self.goal_match_index[agent.id]]])
					# # self.delta_dists = self._bipartite_min_dists(self.dists)
					# # print("delta_dists",self.delta_dists)
