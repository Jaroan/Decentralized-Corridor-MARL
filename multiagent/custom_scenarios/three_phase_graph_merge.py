...
class Scenario(BaseScenario):
	...
	def setup_two_tube_params(self, world):
		...
		tube0 = self._build_tube_params(
			entrance0, exit0, tube_width, tube0_angle,
			np.linalg.norm(exit0 - entrance0), rotation_matrix0
		)
		...
		tube1 = self._build_tube_params(
			entrance1, exit1, tube_width, tube1_angle,
			np.linalg.norm(exit1 - entrance1), rotation_matrix1
		)
	...
	def reward(self, agent: Agent, world: World) -> float:
		...
		elif current_phase == 2 and self.current_tube[agent.id] in (0, 1):  # Transitioned out of tube but not yet reached goal
			# print("Agent", agent.id,"moving to next tube")
			self.current_tube[agent.id] = 2
			self.phase_reached[agent.id] = 0
			current_phase = 0
			agent.previous_phase = 0
			print("Exit", world.tube_params[self.current_tube[agent.id]]['exit'])
			set_landmarks_in_point(self, world, tube_angle=world.tube_params[self.current_tube[agent.id]]['angle'], tube_endpoints=world.tube_params[self.current_tube[agent.id]]['exit'])
		elif current_phase == 2 and self.current_tube[agent.id] == 2:  # Post-tube phase
