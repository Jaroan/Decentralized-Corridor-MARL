...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		# Pre-allocate numpy arrays for frequently used calculations
	...
	def get_rotated_position_from_relative(self, relative_position: np.ndarray,
	...
	def calculate_distances(positions1: np.ndarray, positions2: np.ndarray) -> np.ndarray:
		"""Optimized distance calculation between two sets of positions"""
		n1, n2 = len(positions1), len(positions2)
		distances = np.empty((n1, n2))
		for i in numba.prange(n1):
			for j in range(n2):
				distances[i, j] = np.sqrt(np.sum((positions1[i] - positions2[j]) ** 2))
		return distances

	@staticmethod
	@jit(nopython=True)
	def is_collision_numba(pos1: np.ndarray, pos2: np.ndarray, size1: float, size2: float) -> bool:
		"""Optimized collision detection"""
		dist = np.sqrt(np.sum((pos1 - pos2) ** 2))
		return dist < 1.05 * (size1 + size2)

	@staticmethod
	@jit(nopython=True)
	def get_rotated_position(relative_position: np.ndarray, reference_heading: float) -> np.ndarray:
		"""Optimized rotation calculation"""
		cos_theta = np.cos(reference_heading)
		sin_theta = np.sin(reference_heading)
		rot_matrix = np.array([[cos_theta, sin_theta], [-sin_theta, cos_theta]])
		return np.dot(rot_matrix, relative_position)
	...
	def reward(self, agent: Agent, world: World) -> float:
		...
		if current_phase == 0:  # Pre-tube phase
			...
			# print("Phase 0 dist_to_entrance",dist_to_entrance)
