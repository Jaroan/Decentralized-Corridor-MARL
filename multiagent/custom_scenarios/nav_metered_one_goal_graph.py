...
class Scenario(BaseScenario):
	...
	def random_scenario(self, world):
		...
		if self.formation_type == 'line':
			...
			set_landmarks_in_point(self, world, tube_endpoints=world.tube_params['exit'])
	...
	def reward(self, agent: Agent, world: World) -> float:
		...
		if current_phase == agent.previous_phase+1 and self.phase_reached[agent.id] == current_phase-1:
			...
			if current_phase == 1 and agent.state.p_pos[1] >= (world.tube_params['entrance'][1]-0.1*tube_length):
