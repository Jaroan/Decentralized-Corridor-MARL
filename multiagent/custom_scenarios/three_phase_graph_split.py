...
class Scenario(BaseScenario):
	...
	def random_scenario(self, world):
		...
		while True:
			...
			# Get the current tube from the list
			angle = current_tube['angle']
			
			perp_dir = np.array([np.sin(angle), np.cos(angle)])
			distance_from_entrance = (self.world_size + num_agents_added) / 3
			random_pos = current_tube['entrance'] + distance_from_entrance * perp_dir + jitter
	...
	def setup_two_tube_params(self, world):
		...
		# Center the first tube vertically
		world_center = np.array([0, 0])
		base_entrance1 = np.array([0, tube1_length/4])
		base_exit1 = np.array([0, -tube1_length/4])
		
		# Create rotation matrix for tube 1
		rotation_matrix1 = np.array([
			[np.cos(tube1_angle), np.sin(tube1_angle)],
			[-np.sin(tube1_angle), np.cos(tube1_angle)]
		])
		
		entrance1 = world_center + rotation_matrix1 @ base_entrance1
		exit1 = world_center + rotation_matrix1 @ base_exit1
		
		# Build tube 1 parameters
		tube1 = self._build_tube_params(
			entrance1, exit1, tube_width, tube1_angle, tube1_length, rotation_matrix1
		)
		world.tube_params.append(tube1)
		
		# === TUBE 2: Horizontal tube (perpendicular to tube 1) ===
		...
		offset_distance = self.world_size / 6
		entrance2 = exit1 + np.array([
			offset_distance,
			-offset_distance
		])
		
		# Exit of tube 2 extends horizontally
		direction = 1
		exit2 = entrance2 + np.array([direction * tube2_length, 0])
		
		# Create rotation matrix for tube 2
		rotation_matrix2 = np.array([
			[np.cos(tube2_angle), np.sin(tube2_angle)],
			[-np.sin(tube2_angle), np.cos(tube2_angle)]
		])
		
		# Build tube 2 parameters
		tube2 = self._build_tube_params(
			entrance2, exit2, tube_width, tube2_angle, 
			np.linalg.norm(exit2 - entrance2), rotation_matrix2
		)
		world.tube_params.append(tube2)
		

		# === TUBE3: Horizontal tube (perpendicular to tube 1) ===
		# Randomly choose left or right direction
		# self.tube_choice = np.random.choice([0, 1])
		
		tube3_length = self.world_size * 0.3  # Slightly shorter second tube
		tube3_angle = np.pi/2  # Horizontal
		
		# Entrance of tube 3 starts near exit of tube 1 with some offset
		offset_distance = self.world_size / 6
		entrance3 = exit1 + np.array([
			-offset_distance,
			-offset_distance
		])
		
		# Exit of tube 3 extends horizontally
		direction = -1
		exit3 = entrance3 + np.array([direction * tube3_length, 0])
		
		# Create rotation matrix for tube 3
		rotation_matrix3 = np.array([
			[np.cos(tube3_angle), np.sin(tube3_angle)],
			[-np.sin(tube3_angle), np.cos(tube3_angle)]
		])
		
		# Build tube 3 parameters
		tube3 = self._build_tube_params(
			entrance3, exit3, tube_width, tube3_angle, 
			np.linalg.norm(exit3 - entrance3), rotation_matrix3
		)
	...
	def reward(self, agent: Agent, world: World) -> float:
		...
		if current_phase == 0:  # Pre-tube phase
			...
			self.current_tube[agent.id] = np.random.choice([1, 2])  # randome between 1 and 2 
			self.phase_reached[agent.id] = 0
			current_phase = 0
			agent.previous_phase = 0
			set_landmarks_in_point_seq(self, world, tube_endpoints=world.tube_params[self.current_tube[agent.id]]['exit'], agent_id=agent.id, tube_choice=self.current_tube[agent.id], max_tubes=self.max_tubes)
			...
			if dist_to_goal < self.min_dist_thresh:
				...
				if agent.status is False:
					...
					rew += self.goal_rew*5
					# self.current_tube[agent.id] = (self.current_tube[agent.id] + 1)
