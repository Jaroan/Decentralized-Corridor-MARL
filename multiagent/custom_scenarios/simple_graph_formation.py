...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		####################
		world = World(dynamics_type=self.dynamics_type,separation_distance=self.separation_distance)
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		if world.formation_dist[agent.id] < 1.05*(self.target_radius) and world.formation_dist[agent.id] > 0.95*(self.target_radius):
			...
			## TODO: Case where agent veers off from the goal and then comes back to the goal
	...
	def calculate_circumcenter(self, own_position, neighbor1_pos, neighbor2_pos):
		"""
	...
	def calculate_reward(self, circumcenter, original_center, current_pos):
		"""
	...
	target_center = (2, 4)  # The actual center of the desired circle

	circumcenter = compute_circumcenter(robot_position, neighbor_1, neighbor_2)
	reward = reward_function(circumcenter, target_center)

	print("Computed Circumcenter:", circumcenter)
	print("Reward:", reward)
