...
class Scenario(BaseScenario):
	def make_world(self, args:argparse.Namespace) -> World:
		...
		self.target_radius = 0.5*self.world_size # fixing the target radius for now
	...
	def random_scenario(self, world):
		...
		# Generate thetas for each group separately
		thetas1 = np.linspace(0, np.pi-0.1, half_agents, endpoint=False)
		thetas2 = np.linspace(np.pi+0.5, 2*np.pi, self.num_agents - half_agents, endpoint=False)  # Offset by a small amount
		# Combine the thetas
		self.thetas = np.concatenate([thetas1, thetas2])
		normalized_angles = np.mod(self.thetas + np.pi, 2*np.pi) - np.pi
		self.thetas = normalized_angles
		...
		self.expected_poses = np.array([
			...
			* np.array(
				[
					np.sin(theta) / (1 + np.cos(theta)**2) * 1.5,  # x = sin(t) / (1 + cos(t)^2)
					np.sin(theta) * np.cos(theta) / (1 + np.cos(theta)**2) * 2,  # y = sin(t) * cos(t) / (1 + cos(t)^2)
				]
			)
	...
	def info_callback(self, agent:Agent, world:World) -> Tuple:
		...
		lobe1_center = world.landmarks[0].state.p_pos + np.array([self.target_radius/2, 0.0])
		lobe2_center = world.landmarks[0].state.p_pos + np.array([-self.target_radius/2, 0.0])
		# print("Agent", agent.id,world.landmarks[0].state.p_pos,"lobe1_center",lobe1_center,"lobe2_center",lobe2_center)
		# print("Agent", agent.id, agent.status)
		# Calculate the distance to the central landmark
		distance_to_center1 = np.linalg.norm(agent.state.p_pos - lobe1_center)
		distance_to_center2 = np.linalg.norm(agent.state.p_pos - lobe2_center)
		dist = np.linalg.norm(agent.state.p_pos - world.landmarks[0].state.p_pos)
		# print("distance_to_center1",distance_to_center1,"distance_to_center2",distance_to_center2,"dist to center", dist, 1.5*(self.target_radius/2))
		# print("distance_to_center",agent.state.p_pos)
		# # Calculate the angular position of the agent relative to the central landmark
		# angle = np.arctan2(agent.state.p_pos[1] - world.landmarks[0].state.p_pos[1], 
		# 				agent.state.p_pos[0] - world.landmarks[0].state.p_pos[0])
		# if agent.id == 1:
		# 	print("angle",angle,"thetas",self.thetas)
		# # Check if the angle is present in self.thetas
		# if np.any(np.isclose(self.thetas, angle, atol=0.1)):  # adjust the tolerance as needed
		# 	# Calculate the target position on the infinity shape for this angle
		# 	target_pos = world.landmarks[0].state.p_pos + self.target_radius * np.array(
		# 		[
		# 			np.sin(angle) / (1 + np.cos(angle)**2) * 1.5,  # x = sin(t) / (1 + cos(t)^2)
		# 			np.sin(angle) * np.cos(angle) / (1 + np.cos(angle)**2) * 2,  # y = sin(t) * cos(t) / (1 + cos(t)^2)
		# 		]
		# 	)
		# 	print("!!!!!!target_pos",target_pos,"agent_pos",agent.state.p_pos)
			# Check if the agent is within a certain range of its target position
			# if np.linalg.norm(agent.state.p_pos - target_pos) < self.min_dist_thresh:
		if agent.status == False:
			world.times_required[agent.id] = -1
		elif agent.status == True:
			# print("Agent", agent.id, "reached the goal")
			world.times_required[agent.id] = world.current_time_step * world.dt
			world.dists_to_goal[agent.id] = agent.state.p_dist
			# print("world.times_required[agent.id]",world.times_required[agent.id],"world.dists_to_goal[agent.id]",world.dists_to_goal[agent.id])
		if (0.8*(self.target_radius/2) < distance_to_center1 < 1.2*(self.target_radius/2)) or (0.8*(self.target_radius/2) < distance_to_center2 < 1.2*(self.target_radius/2)):
			world.formation_complete[agent.id] = 1
			# print("dist", world.times_required[agent.id],distance_to_center1, distance_to_center2, 1.2*(self.target_radius/2))
