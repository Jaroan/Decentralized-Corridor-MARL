...
class Runner(object):
	...
	def process_infos(self, infos):
		...
		for agent_id in range(self.num_agents):
			...
			for info in infos:
				...
				if 'Conformance' in info[agent_id].keys():
					# print("conformance is", info[agent_id]['Conformance'])
					conformance_percentages_list.append(info[agent_id]['Conformance'])
				if 'Delta_spacing' in info[agent_id].keys():
	...
	# create get_conformation_percentages() function
	def get_conformation_percentages(self, env_infos:Dict):
		"""
			Get the confromation success info from env_infos
			Example: {'agent0/formation_dist': [5], 
					'agent1/formation_dist': [5]}
		"""
		conformation_percentages = []
		for k, v in env_infos.items():
			if 'conformance_percentages' in k:
				# print("formation dist", v)
				conformation_percentages.append(v[0])

		# print("formation success", formation_success)
		return conformation_percentages
	
	def get_delta_spacing(self, env_infos:Dict):
		"""
			Get the delta spacing info from env_infos
			Example: {'agent0/formation_dist': [5], 
					'agent1/formation_dist': [5]}
		"""
		delta_spacing = []
		for k, v in env_infos.items():
			if 'delta_space' in k:
				# print("formation dist", v)
				delta_spacing.append(v[0])

		# print("formation success", formation_success)
		return delta_spacing

	def get_spacing_violations(self, env_infos:Dict):
		"""
			Get the spacing violations info from env_infos
			Example: {'agent0/formation_dist': [5], 
					'agent1/formation_dist': [5]}
		"""
		space_violations = []
		for k, v in env_infos.items():
			if 'spacing_violations' in k:
				# print("formation dist", v)
				space_violations.append(v[0])
