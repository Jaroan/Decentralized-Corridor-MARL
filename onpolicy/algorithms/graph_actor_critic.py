...
from onpolicy.algorithms.utils.gnn_new import GNNBase
...
def minibatchGenerator(obs:Tensor,
                    node_obs:Tensor, 
                    adj:Tensor, 
                    agent_id:Tensor,
                    max_batch_size:int):
    ...
    for i in range(num_minibatches):
        yield (obs[i*max_batch_size:(i+1)*max_batch_size],
...
class GR_Actor(nn.Module):
    ...
    def __init__(self, 
        ...
        # print("obs_space", obs_space.shape, obs_space)
        # print("node_obs_space",node_obs_space.shape,node_obs_space)
        ...
        self.base = MLPBase(args, obs_shape=None, 
                            override_obs_dim=mlp_base_in_dim)
    ...
    def forward(self, 
                ...
                deterministic=False) -> Tuple[Tensor, Tensor, Tensor]:
        """
        ...
        # print("node obs", node_obs.shape,node_obs)
        ...
        if available_actions is not None:
            ...
            # print("ACTORCRITICSavailable_actions", available_actions)
        ...
        if (self.split_batch) and (obs.shape[0] > self.max_batch_size):
            ...
            batchGenerator = minibatchGenerator(obs, node_obs, 
                                                adj, agent_id, 
                                                self.max_batch_size)
            actor_features = []
            for batch in batchGenerator:
                obs_batch, node_obs_batch, adj_batch, agent_id_batch = batch
                nbd_feats_batch = self.gnn_base(node_obs_batch, adj_batch, 
                                                agent_id_batch)
    ...
    def evaluate_actions(self, 
                        ...
                        masks, 
                        available_actions=None, 
                        active_masks=None) -> Tuple[Tensor, Tensor]:
        """
        ...
        if available_actions is not None:
            available_actions = check(available_actions).to(**self.tpdv)
        # print("ACTIVE MASKS", active_masks.T)
        if active_masks is not None:
            # print("Active", active_masks.T)
...
class GR_Critic(nn.Module):
    ...
    def forward(self, 
        ...
        if (self.split_batch) and (cent_obs.shape[0] > self.max_batch_size):
            ...
            batchGenerator = minibatchGenerator(cent_obs, node_obs, 
                                                adj, agent_id, 
                                                self.max_batch_size)
            critic_features = []
            for batch in batchGenerator:
