...
def get_config():
    ...
    Save & Log parameters:
        ...
        --img_interval <int>
            time duration between contiunous twice img saving.
    ...
    parser.add_argument("--formation_type", type=str,
                        default='random', help="Formation type for agents")
    ## formation rew
    ...
    parser.add_argument("--num_agents_per_team", type=int,
                        default=3, help="num agents per team")
    parser.add_argument("--num_teams", type=int,
                        default=1, help="num of teams")
    parser.add_argument("--zeroshift", type=int,
                        default=5, help="zero shift for relu, tanh etc")
    parser.add_argument("--increase_fairness", type=lambda x:bool(strtobool(x)), default=False,
    ...
    # env parameters
    parser.add_argument("--env_name", type=str, default='GraphMPE', 
    ...
    parser.add_argument("--world_size", type=float, default=2,
    ...
    parser.add_argument('--num_walls', type=int, default=0,
                        help="Number of walls in the environment")
    ...
    parser.add_argument("--use_valuenorm", action='store_false', default=True, 
                        help="by default True, use running mean and std "
    ...
    parser.add_argument("--split_batch", action='store_true', default=False,
                        help="Whether to split batch for actor/critic networks")
    parser.add_argument("--max_batch_size", type=int, default=32,
                        help="The max batch size for training. Only matters when "
                        "--split_batch=True")
    ...
    parser.add_argument("--use_clipped_value_loss", action='store_false', 
                        default=True, help="by default, clip loss value. "
                        "If set, do not clip loss value.")
    ...
    parser.add_argument("--use_value_active_masks", action='store_false', 
                        default=True, help="by default True, whether to mask "
                        "useless data in value loss.")
    parser.add_argument("--use_policy_active_masks", action='store_false', 
                        default=True, help="by default True, whether to mask "
                        "useless data in policy loss.")
    ...
    # img parameters
    parser.add_argument("--img_interval", type=int, default=2, 
                        help="time duration between contiunous twice img saving.")
    # eval parameters
    ...
    parser.add_argument("--use_render", action='store_true', default=False, 
                        ...
                        "this hyperparam.")
    parser.add_argument("--render_episodes", type=int, default=None, 
...
def graph_config(args, parser):
    ...
    embedding_size = (num_entities)//2
    # print("***************************************************************")
    # print(f"Embedding size: {embedding_size}")
    parser.add_argument('--num_embeddings', type=int, default=num_entities,
    ...
    parser.add_argument('--embed_hidden_size', type=int, default=16,
                        help="Hidden layer dimension after the embedding layer")
    parser.add_argument('--embed_layer_N', type=int, default=1,
                        help="Number of hidden linear layers after the "
                        "embedding layer")
    parser.add_argument('--embed_use_ReLU', action="store_false", default=True,
                        help="Whether to use ReLU in the linear layers after "
                        "the embedding layer")
    parser.add_argument('--embed_add_self_loop', action="store_true", default=False,
                        help="Whether to add self loops in adjacency matrix")
    parser.add_argument('--gnn_hidden_size', type=int, default=16, 
    ...
    parser.add_argument('--gnn_concat_heads', action="store_true", default=False,
    ...
    parser.add_argument('--gnn_use_ReLU', action="store_false", default=True,
                        help="Whether to use ReLU in GNN conv layers")
    # parser.add_argument('--max_edge_dist', type=float, default=1, 
    #                     help="Maximum distance above which edges cannot be "
    #                     "connected between the entities")
    parser.add_argument('--graph_feat_type', type=str, default='relative', 
                        choices=['global', 'relative'], help="Whether to use "
                        "'global' node/edge feats or 'relative'")
    parser.add_argument('--actor_graph_aggr', type=str, default='node',
                        choices=['global', 'node'], help='Whether we want to '
                        'pull node specific features from the output or perform '
                        'global_pool on all nodes. ')
    parser.add_argument('--critic_graph_aggr', type=str, default='global',
                        choices=['global', 'node'], help='Whether we want to '
                        'pull node specific features from the output or perform '
                        'global_pool on all nodes. ')
    parser.add_argument('--global_aggr_type', type=str, default='mean', 
                        choices=['mean', 'max', 'add'], help='The type of '
                        'aggregation to perform if `graph_aggr` is `global`')
    parser.add_argument('--use_cent_obs', type=lambda x:bool(strtobool(x)), 
                        default=False, help="Whether to use centralized observation "
                        "for critic or not")
    parser.add_argument("--auto_mini_batch_size", action='store_true', default=False,
                        help="Whether to automatically set mini batch size")
    parser.add_argument("--target_mini_batch_size", type=int, default=32,
    ...
    if all_args.auto_mini_batch_size:
        ...
        num_mini_batch = (all_args.n_rollout_threads * all_args.episode_length * \
            all_args.num_agents) // (all_args.target_mini_batch_size)
        new_batch_size = (all_args.n_rollout_threads * all_args.episode_length * \
            all_args.num_agents) // num_mini_batch // all_args.data_chunk_length * \
