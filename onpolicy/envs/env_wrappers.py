...
class ShareVecEnv(ABC):
    ...
    def __init__(self, num_envs, observation_space, 
    ...
    def step(self, actions, num_current_episode: Optional[int] = None):
        """
        Step the environments synchronously.

        This is available for backwards compatibility.
        """
        self.step_async(actions, num_current_episode)
...
class GuardSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        observation_space, share_observation_space, \
...
class ShareSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=shareworker, 
                    args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                    for (work_remote, remote, env_fn) in 
                    zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
...
class ChooseSimpleSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=choosesimpleworker, 
                    args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                    for (work_remote, remote, env_fn) in 
                    zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
...
class ChooseSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=chooseworker, 
                    args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                    for (work_remote, remote, env_fn) in 
                    zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
...
class ChooseGuardSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=chooseguardworker, 
                    args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                    for (work_remote, remote, env_fn) in 
                    zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
...
class ChooseDummyVecEnv(ShareVecEnv):
    ...
    def reset(self, reset_choose):
        results = [env.reset(choose) for (env, choose) 
...
def graphworker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        cmd, data = remote.recv()
        if cmd == 'step':
            # print("Worker executing reset") 
            actions = data[0]
            num_current_env_steps = data[1]
            ob, ag_id, node_ob, adj, reward, done, info = env.step(actions)
            # print(f"Worker reset result: {ob}, {ag_id}, {node_ob}, {adj}") 
            # ob, ag_id, node_ob, adj, reward,cooperate,defect, done, info = env.step(data)
            ep_info = {}
            if 'bool' in done.__class__.__name__:
                if done:
                    ob, ag_id, node_ob, adj = env.reset(num_current_env_steps)
            else:
                if np.all(done):
                    ob, ag_id, node_ob, adj = env.reset(num_current_env_steps)
            if ep_info:
            ...
            # remote.send((ob, ag_id, node_ob, adj, reward, cooperate, defect,done, info))
        elif cmd == 'reset':
            ob, ag_id, node_ob, adj = env.reset(data)
...
class GraphDummyVecEnv(ShareVecEnv):
    ...
    def step_async(self, actions, num_current_episode):
        self.actions = actions

    def step_wait(self):
        results = [env.step(a) for (a, env) in zip(self.actions, self.envs)]
        ### for prisoners dilemma
        ### obs, ag_ids, node_obs, adj, rews, cooperate, defect,dones, infos = map(np.array, zip(*results))
        ...
        self.actions = None
        return obs, ag_ids, node_obs, adj, rews, dones, infos, reset_count
        ### for prisoners dilemma
        ### return obs, ag_ids, node_obs, adj, rews,cooperate, defect, dones, infos

    def reset(self, num_current_episode: int = 0):
        results = [env.reset(num_current_episode) for env in self.envs]
...
class GraphSubprocVecEnv(ShareVecEnv):
    def __init__(self, env_fns, spaces=None):
        ...
        nenvs = len(env_fns)
        self.remotes, self.work_remotes = zip(*[Pipe() for _ in range(nenvs)])
        self.ps = [Process(target=graphworker, 
                    args=(work_remote, remote, CloudpickleWrapper(env_fn)))
                    for (work_remote, remote, env_fn) in 
                    zip(self.work_remotes, self.remotes, env_fns)]
        for p in self.ps:
        ...
        agent_id_observation_space, share_agent_id_observation_space = self.remotes[0].recv()
        ShareVecEnv.__init__(self, len(env_fns), observation_space,
                            share_observation_space, action_space)
    ...
    def step_async(self, actions, num_current_episode):
        for remote, action in zip(self.remotes, actions):
            remote.send(('step', (action, num_current_episode)))
        self.waiting = True

    def step_wait(self):
        results = [remote.recv() for remote in self.remotes]
        self.waiting = False
        # obs, ag_ids, node_obs, adj, rews,cooperate, defect, dones, infos = zip(*results)
        obs, ag_ids, node_obs, adj, rews, dones, infos = zip(*results)
        # return (np.stack(obs), np.stack(ag_ids), np.stack(node_obs), np.stack(adj), 
        #         np.stack(rews),np.stack(cooperate),np.stack(defect), np.stack(dones), infos)
        return (np.stack(obs), np.stack(ag_ids), np.stack(node_obs), np.stack(adj), 
                np.stack(rews), np.stack(dones), infos)

    def reset(self, num_current_episode: int = 0):
        # raise NotImplementedError
        # print("GraphSubprocVecEnv reset: ", num_current_episode)
        for remote in self.remotes:
            remote.send(('reset', num_current_episode))
        results = [remote.recv() for remote in self.remotes]
