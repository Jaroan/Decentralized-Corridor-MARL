#!/bin/bash
# Run merge corridor experiments with different agent counts
...
SCENARIO="three_phase_graph_merge"
WORLD_SIZE=5
SEED=0

echo "Running merge corridor experiments..."
echo ""
...
#   --eval_mode \
#   --eval_output_dir="eval_merge_10agents" \
#   --save_trajectories
...
python onpolicy/scripts/eval_mpe.py \
  ...
  --formation_type='point' \
  --eval_mode \
  --eval_output_dir="eval_merge_20agents" \
  --save_trajectories
...
python onpolicy/scripts/eval_mpe.py \
  ...
  --formation_type='point' \
  --eval_mode \
  --eval_output_dir="eval_merge_30agents" \
  --save_trajectories
...
python onpolicy/scripts/eval_mpe.py \
  ...
  --formation_type='point' \
  --eval_mode \
  --eval_output_dir="eval_merge_40agents" \
  --save_trajectories
...
echo "  python eval_scripts/compute_merge_average_speeds.py"
